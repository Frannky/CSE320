...
#include <unistd.h>  // is this allowed?
#include <time.h>    // is this allowed?   allowed
...
//    Bool not_own_rolodex;// boolean 1 is true; 0 is false
    char *user;
    FILE *tempfp;
...
//        fprintf(stderr, "argv[optind] : %s\n" , argv[optind]);
    ...
    //the third argument is dup_error, why it is initialized to true??????
...
//    not_own_rolodex = option_present(OTHERUSERFLAG);
    ...
    if ((rolofd = open(homedir(ROLODATA),O_RDONLY)) == -1 ) { // instead of null < 0 correct ??????
    ...
    if (people_flag && optind < argc) {
        print_people(optind, argc , argv);
        exit(0);
    }
