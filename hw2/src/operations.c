...
//    fprintf(stderr,"MENU_YES = %d\n", MENU_YES);
//    fprintf(stderr,"DEFAULT_NO = %d\n", DEFAULT_NO);
...

{
  ...
  //long timeval;  not used
  char *response;
  char *basicdata[N_BASIC_FIELDS], *otherdata[100]; // did not use *datum;
  Ptr_Rolo_List rlink;
      ...
      switch (rval) {
        ...
        case MENU_MATCH :
          switch (menuval) {
            ...
            case A_ABORT_ADD :
                // need to free basic data here
                for(int ind = 0 ; ind <= j ; ind++){
                    if(basicdata[ind] != NULL && strlen(basicdata[ind]) > 0) free(basicdata[ind]);
                }
              return;
              break;
            case A_FILL_IN_REST :
         ...
         switch (rval) {
           case MENU_MATCH :
             switch (menuval) {
               ...
               case O_ABORT :
                   // need to free basic data and other data here
                   for(int ind = 0 ; ind < N_BASIC_FIELDS ; ind++){
                       if(basicdata[ind] != NULL && strlen(basicdata[ind]) > 0) free(basicdata[ind]);
                   }
                     for(int ind = 0; ind < j; ind++){
                         if(otherdata[ind] != NULL && strlen(otherdata[ind]) > 0) free(otherdata[ind]);
                     }
                 return;
                 break;
               case O_HELP :
