...
//        char* st1 = copystr(get_basic_rolo_field(j,entry));
//        set_basic_rolo_field(j,new_entry,st1);
//        free(st1);
...
//    free(st3);
    ...
    if (n > 0) {
        ...
        for (j = 0; j < n; j++) {
            ...
            //char* st2 = copystr(get_basic_rolo_field(j,entry));
            //set_basic_rolo_field(j,new_entry,st2);
            //free(st2);
        }
//        free(otherfields);
    }else{
        ...
        case MENU_MATCH :
            ...
            switch (menuval) {
                ...
                case U_ABORT :
                    ...
                    for(int ind = 0 ; ind < N_BASIC_FIELDS; ind++){
//
                        if(entry->basicfields[ind]) {
//                            fprintf(stderr,"%s\n",entry->basicfields[ind]);
                            free(entry->basicfields[ind]);
                        }
                    }
                    for(int ind = 0 ; ind < entry->n_others ; ind++){
                        if(entry->other_fields[ind]) {
//                            fprintf(stderr,"%s\n",entry->other_fields[ind]);
                            free(entry->other_fields[ind]);
                        }
                    }
//                    for(int ind = 0 ; ind < N_BASIC_FIELDS; ind++){
////
//                        if(old_entry->basicfields[ind] && strlen(old_entry->basicfields[ind])>0) {
//                            free(old_entry->basicfields[ind]);
//                        }
//                    }
//                    for(int ind = 0 ; ind < entry->n_others ; ind++){
//                        if(old_entry->other_fields[ind] && strlen(old_entry->other_fields[ind])>0) free(old_entry->other_fields[ind]);
//                    }
...
//                    free(rlink);
                ...
                case U_END_UPDATE :
                    ...
                    if (MENU_YES == rolo_menu_yes_no (
                        ...
                        changed = 1;
//                        for(int ind = 0 ; ind < N_BASIC_FIELDS ; ind++){
//                            if(old_entry->basicfields[ind]) free(old_entry->basicfields[ind]);
//                        }
//                        for(int ind = 0 ; ind < entry->n_others ; ind++){
//                            if(old_entry->other_fields[ind]) free(old_entry->other_fields[ind]);
//                        }
//                        for(int ind = 0 ; ind < N_BASIC_FIELDS ; ind++){
//                            if(entry->basicfields[ind] != NULL) free(entry->basicfields[ind]);
//                        }
//                        for(int ind = 0 ; ind < entry->n_others ; ind++){
//                            if(entry->other_fields[ind] != NULL) free(entry->other_fields[ind]);
//                        }
                        free(old_entry->other_fields);
                        free(old_entry);
//                        free(entry->other_fields);
//                        free(entry);
                        ...
                        updated = 0;
//                        for(int ind = 0 ; ind < N_BASIC_FIELDS ; ind++){
//                            if(entry->basicfields[ind]) free(entry->basicfields[ind]);
//                        }
//                        for(int ind = 0 ; ind < entry->n_others ; ind++){
//                            if(entry->other_fields[ind]) free(entry->other_fields[ind]);
//                        }
//
//                        for(int ind = 0 ; ind < N_BASIC_FIELDS ; ind++){
//                            if(old_entry->basicfields[ind] != NULL) free(old_entry->basicfields[ind]);
//                        }
//                        for(int ind = 0 ; ind < entry->n_others ; ind++){
//                            if(old_entry->other_fields[ind] != NULL) free(old_entry->other_fields[ind]);
//                        }
//                        free(entry->other_fields);
                        free(old_entry->other_fields);
                        free(old_entry);
...
//                printf("%p", entry->basicfields);
                printf("Old value: %s\n",get_basic_rolo_field(findex,entry));
                printf("New value: ");
                char * value1 = get_new_value();
                if (0 == (newval = copystr(value1))) {
                    free(value1);
                    break;
                }
                ...
                printf("findex = %d", findex);
                printf("New value: ");
                char * value2 = get_new_value();
                if (0 == (newval = copystr(value2))) {
                    free(value2);
                    break;
                }
                free(value2);
                ...
                char* value3 = get_new_value();
                if (0 == (newfield = copystr(value3))) {
                    free(value3);
                    break;
                }
                free(value3);
