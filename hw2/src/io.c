...
{
//  int semi = 0;  what's the meaning here????
    if(value == NULL || name == NULL) return;
    int i;
    if (all_whitespace(value) && !show) return;
    printf("%-25s",name);
    int j = 0;
    while (*(value+j) != '\0') { //*value
        if (*(value+j) == ';') { //*value
            while (*(++j + value) == ' '); // *++value
            putchar('\n');
            for (i = 0; i < (up ? 28 : 25); i++) putchar(' ');
            //semi = 1;what's the meaning here????
        }
        else {
            //semi = 0;what's the meaning here????
            putchar(*(value+ j++)); //*value++
        }
    }
    putchar('\n');
}
...
//        if (string == NULL) {
//          fprintf(stderr,"Fatal error:  empty string\n");
//            save_and_exit(0);
//        }
